...
public interface StatRepository extends CrudRepository<Event, Long> {
    @Query("SELECT new ru.practicum.ewm.server.stats.entity.EventForGet(event.app, event.uri, COUNT(ip))\n" +
            "FROM Event as event\n" +
    ...
    @Query("SELECT new ru.practicum.ewm.server.stats.entity.EventForGet(event.app, event.uri, COUNT(DISTINCT ip))\n" +
           "FROM Event as event\n" +
           "WHERE event.existDate BETWEEN ?1 AND ?2\n" +
           "GROUP BY event.app, event.uri")
